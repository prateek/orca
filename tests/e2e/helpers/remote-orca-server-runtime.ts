...
async function waitForServeReady(
  ...
  return await new Promise((resolve, reject) => {
    ...
    const cleanup = (): void => {
      ...
      scenario.child.stderr.off('data', onStderr)
    ...
    const onStdout = (chunk: Buffer): void => {
      scenario.stdout += chunk.toString()
      scanStdout()
    }
    const onStderr = (chunk: Buffer): void => {
      scenario.stderr += chunk.toString()
    }
...
export async function startRemoteOrcaServerScenario(
  testInfo: TestInfo,
  args: {
    key: 'server-a' | 'server-b'
    name: string
  ...
  const ready = await waitForServeReady(scenario)
  return {
    ...scenario,
...
export async function stopRemoteOrcaServerScenario(
  ...
  if (scenario.child.exitCode === null && !scenario.child.killed) {
    scenario.child.kill()
    await new Promise<void>((resolve) => {
      const timer = setTimeout(resolve, 5_000)
      scenario.child.once('exit', () => {
        clearTimeout(timer)
        resolve()
      })
    })
