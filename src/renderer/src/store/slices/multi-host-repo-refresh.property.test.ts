...
function repo(hostId: HostId, index: number, revision = 0): Repo {
  ...
  const id = index === 1 ? 'same-repo-id' : `${hostSlug}-repo-${index}`
  return {
    id,
...
function setupIdentity(setup: ProjectHostSetup): string {
  return `${setup.hostId}\0${setup.repoId}\0${setup.path}`
}

function repoIdentity(entry: Repo): string {
  return `${getRepoExecutionHostId(entry)}\0${entry.id}\0${entry.path}`
}

function uniqueRepoIds(repos: readonly Repo[]): string[] {
  return [...new Set(repos.map((entry) => entry.id))].sort()
}
  ...
  for (const hostId of HOSTS) {
    expect(
      repos
        ...
        .map((entry) => `${entry.id}:${entry.displayName}:${entry.path}`)
        .sort()
    ).toEqual(
      truthByHost[hostId].map((entry) => `${entry.id}:${entry.displayName}:${entry.path}`).sort()
    )
  }
}
...
function assertCompatibilityMatchesRepos(repos: readonly Repo[]): void {
  ...
  const setupIdentities = compatibility.projectHostSetups.map(setupIdentity).sort()
  expect(setupIdentities).toEqual(repos.map(repoIdentity).sort())
  expect(
    compatibility.projects.find((project) => project.id === PROJECT_ID)?.sourceRepoIds.toSorted()
  ).toEqual(uniqueRepoIds(repos))
}
...
describe('multi-host repo refresh properties', () => {
  it('preserves every host partition across generated refresh sequences', () => {
    for (let seed = 1; seed <= 25; seed += 1) {
      ...
      expect(HOSTS.map((hostId) => truthByHost[hostId][0]?.id)).toEqual([
        'same-repo-id',
        'same-repo-id',
        'same-repo-id'
      ])
