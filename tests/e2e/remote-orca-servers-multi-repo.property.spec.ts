...
async function addServerRepoFromDesktop(
  ...
  const slug = `${server.key}-${sharedProject ? 'shared-orca' : 'side'}-${step}`
  ...
  for (let seed = 1; seed <= SEED_COUNT; seed += 1) {
    test(`preserves desktop state across paired Remote Orca Servers, seed ${seed}`, async ({
      ...
      try {
        ...
        const localPath = createLocalGitRepo(localRoot, 'local-shared-orca', {
        ...
        servers.push(
          await startRemoteOrcaServerScenario(testInfo, {
            key: 'server-a',
        ...
        await addRemoteRepoFromDesktop(orcaPage, servers[0], {
          slug: 'server-a-shared-orca',
          sharedProject: true
        })
        await addRemoteRepoFromDesktop(orcaPage, servers[1], {
          slug: 'server-b-shared-orca',
          sharedProject: true
        })
        await addRemoteRepoFromDesktop(orcaPage, servers[1], {
