...
export async function removeRemoteOrcaServerFromDesktop(
  page: Page,
  ...
  await page.evaluate(async (id) => {
    ...
    if (store?.getState().settings?.activeRuntimeEnvironmentId === id) {
      const switched = await store.getState().switchRuntimeEnvironment?.(null)
      if (switched === false) {
        throw new Error(`Failed to switch away from removed runtime environment: ${id}`)
      }
    }
    await window.api.runtimeEnvironments.remove({ selector: id })
    if (!store) {
      return
    }
    const hostId = `runtime:${id}`
    const nextRepos = store.getState().repos.filter((repo) => repo.executionHostId !== hostId)
    const remainingRepoIds = new Set(nextRepos.map((repo) => repo.id))
    store.setState((state) => ({
      repos: nextRepos,
      projectHostSetups: state.projectHostSetups.filter((setup) => setup.hostId !== hostId),
      projects: state.projects
        .map((project) => ({
          ...project,
          sourceRepoIds: project.sourceRepoIds.filter((repoId) => remainingRepoIds.has(repoId))
        }))
        .filter((project) => project.sourceRepoIds.length > 0)
    }))
  }, environmentId)
...
export async function renameRemoteRepoFromDesktop(
  ...
  const repo = await page.evaluate(
    async ({ hostId, id, name }) => {
      ...
      const repo = store
        .getState()
        .repos.find((candidate) => candidate.id === id && candidate.executionHostId === hostId)
      if (!repo) {
        throw new Error(`Expected renamed remote repo to stay loaded: ${hostId}:${id}`)
      }
      return repo
    },
    { hostId: `runtime:${scenario.environmentId}`, id: repoId, name: displayName }
  )
  rememberRemoteScenarioRepo(scenario, repo)
}
